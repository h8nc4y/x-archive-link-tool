...
- [ ] `RATE_LIMIT_PER_IP_PER_MINUTE`: 初期運用値は `10`
- [ ] `RATE_LIMIT_GLOBAL_PER_MINUTE`: 初期運用値は `60`
...
- [ ] `RATE_LIMIT_PER_IP_PER_MINUTE`: 初期運用値は `10`。Cloudflare Production環境変数への反映有無は未確認。
- [ ] `RATE_LIMIT_GLOBAL_PER_MINUTE`: 初期運用値は `60`。Cloudflare Production環境変数への反映有無は未確認。
- [ ] 問い合わせ先: TODO/未設定。
- [ ] プライバシーポリシーURL: TODO/未設定。
- [ ] Cloudflare Pages deploy status確認: 人間がCloudflare Pages Deploymentsで最新commitのProduction成功を確認する。自動deploy結果は未確認のままにしない。

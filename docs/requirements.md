...
- Xポスト共有URLから貼り付け用テキストを生成する**記録補助ツール**（2026-07-04 オーナー決定）。
- 法的な証拠保全を保証するツールではない。UI・README・プライバシーポリシーで「スクリーンショットやPDF保存の併用を推奨」と明記する。
- 公開範囲は**M3（一般公開）**。公開URLは `https://x-archive-link-tool.pages.dev`（Cloudflare Pages、Production branch = `master`）。
- API方針は**oEmbed-first**を維持する（X API v2はBYOTで任意。規約適合の無料メディアURL取得手段は2026-07-04調査で「なし」と結論。方針変更はゲート④=製品要件変更）。
...
- 現在のタスクと優先度は `TASKS_BACKLOG.md`（リポジトリルート）を正とする。
- 本書の要件を変更する場合はゲート④（製品要件変更＝人間承認）に該当する。オーナー決定を得てから本書と `docs/DECISION_LOG.md` を更新する。

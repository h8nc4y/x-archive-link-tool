...
- 生成したPNGは、ユーザーが明示的にボタン操作した場合だけ、imgurへ匿名アップロードして共有URLを取得できる（imgur Client-ID方式）。
- imgur Client-IDが未設定の間はアップロード機能を無効化し、準備中である旨を表示する。
- アップロードはサーバーを経由せず、ブラウザから直接imgur APIへ送信する。

...
Cloudflare Pages Functionsの環境にR2バケットbinding `RECORD_IMAGE_BUCKET` を通じて
...
```json
{ "url": "https://example.pages.dev/i/1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d" }
```

`url` は自ドメインの配信エンドポイント（後述の `GET /i/{id}`）。

### エラー

| HTTP | code | 意味 |
| --- | --- | --- |
| 503 | `upload_not_configured` | R2バケットbinding `RECORD_IMAGE_BUCKET` が未設定 |
...
| 429 | `upload_429` | rate limit超過 |
| 502 | `upload_error` | R2書き込み失敗 |
...
errorCode）のみを残す。発行したid・配信URL・画像内容はログに残さない。

## GET /i/{id}

`/api/upload-image` が発行した記録画像の配信エンドポイント。
...
- `/api/extract` が `oembed_unreachable` を返した場合、クライアント（`apps/web/app.js`）は publish.x.com の公式oEmbedへブラウザから直接アクセスするフォールバックを試みる（2026-07-07 オーナー承認・設計A）。Cloudflare Workersからのfetchが X 側のIP遮断で失敗する対策で、直接フォールバックの結果は `source: "oembed-direct"` として画像・動画URLとユーザー数値ID非取得の旨をUIに明記する。
...
- Twitter魚拓（twtr.satoru.net、GETフォーム相当のdeep link）: `https://twtr.satoru.net/?mode=check&url={encodeURIComponent(canonicalXPostUrl)}`
...
<ul class="archive-links" aria-label="魚拓サービス">
  ...
  <li><a id="twtr-link" class="archive-link" target="_blank" rel="noopener" aria-disabled="true">Twitter魚拓（twtr.satoru.net）で画像魚拓を取る</a></li>
</ul>
```

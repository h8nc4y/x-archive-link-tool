...
- `X_BEARER_TOKEN` 設定時にX APIが失敗し、oEmbed fallbackが成功した場合、現行実装ではそのoEmbed由来レスポンスも通常の成功レスポンスとしてcache保存される。非cache、短TTL、現状維持のどれを正式方針にするかは未決であり、このdocs-only整理では挙動を変更しない。
...
- KV physical TTLをlogical TTLより長くする案は、privacy/legal/retention判断が必要な未決事項。このdocs-only整理ではTTL、cache key、fallback cache policyを変更しない。

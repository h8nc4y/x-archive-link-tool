...
This file summarizes repository state and the AI review-coordination documents as of 2026-05-29. It is based on local repository inspection and safe read-only commands. It does not claim that Claude Code has already reviewed the repository, and it does not claim that ChatGPT has already triaged any Claude findings.

Current-status note as of the 2026-05-30 follow-up: this file is retained as a historical pre-review handoff packet. Claude review output has since been recorded in `docs/CLAUDE_REVIEW.md`, and ChatGPT triage has approved only CL-003, CL-005, CL-010 regression tests for those two items, and review-management docs for the current implementation pass. Treat `docs/AI_REVIEW_TRIAGE.md` and `docs/CODEX_TASKS.md` as the current approval and implementation scope.
...
- Working tree status at inspection time for this handoff:
...
- Claude review has not happened yet in these coordination docs.
- ChatGPT triage has not happened yet in these coordination docs.
- Codex has no approved Claude-review implementation tasks yet.
...
- The next useful AI step is a read-only Claude Code review prompt generated by ChatGPT, not direct implementation.
...
The immediate coordination goal is: give ChatGPT enough context to create a Claude Code prompt for an independent read-only review, then let ChatGPT triage Claude findings before Codex implements anything.
...
- No database layer.
- No user auth.
- No current Claude review findings.
- No ChatGPT-approved implementation tasks from Claude findings.
...
- `docs/CODEX_TASKS.md`: originally a placeholder for ChatGPT-approved Codex tasks; now records only the approved CL-003, CL-005, and related regression-test task scope.
...
- `docs/AI_REVIEW_TRIAGE.md`: records ChatGPT triage for this pass. Only CL-003, CL-005, and CL-010 regression coverage for those two findings are approved for implementation.
- `docs/CODEX_TASKS.md`: records the approved Codex task queue for this pass. Non-approved Claude findings remain out of implementation scope.
...
- `type docs\CLAUDE_REVIEW.md`
  - Confirmed Claude review is pending.
- `type docs\AI_REVIEW_TRIAGE.md`
  - Confirmed ChatGPT triage is pending.
- `type docs\CODEX_TASKS.md`
  - Confirmed no approved review-fix task queue exists yet.
...
- Whether ignored local `CLAUDE.md` should be included in Claude context or replaced by `docs/claude-code-usage.md` plus `AGENTS.md`.
...
- Whether stale production-status claims in older docs need a separate verification pass before Claude review.

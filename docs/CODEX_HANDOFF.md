...
基本セット（`check:all` script が存在する場合）:

- `npm.cmd run check:all`（lint + test + post-release docs guard。lint初回実行前に `npm.cmd install` でdevDependenciesを取得する）
- `git diff --check`

`check:all` が未導入のブランチでは:
...
現在タスクの一次情報は `TASKS_BACKLOG.md`、直近の引き継ぎ状態は `HANDOFF.md`（リポジトリルート）だけを正とする。本ファイルにタスクの進捗を複製しない（陳腐化するため）。CC-IDとPR番号は固定対応ではない。報告やPR本文でPR番号を出す場合はGitHubの実値で確認する。
...
- 一括検証: `npm.cmd run check:all`（lint + test + docs guard）
- テスト: `npm.cmd test`

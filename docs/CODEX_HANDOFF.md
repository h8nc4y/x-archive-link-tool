...
- CC-005: 出力形式拡張（C-07, C-12）。
- CC-006: 入力寛容化（C-09）と高コントラスト（C-08）。C-08は色設計判断が必要なら§10/§12を使う。
- CC-007: Issue #42の判断材料整理。外部通信なしのrepo内docs整備まで。
...
中央devlogへは `D:\Agent\Vault\20_dev-shared\claude-dev-log\daily\YYYY-MM-DD.md` に直接追記する。書けない場合だけ、repo内 `docs\DEVLOG-inbox.md` または最終報告の「開発ログ報告（Obsidian用）」へフォールバックする。

...
test("Cloudflare Pages static headers mirror server security headers", async () => {
  ...
  assert.match(
    headers,
    /Content-Security-Policy:\s*default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self' https:\/\/publish\.x\.com; frame-ancestors 'none'; base-uri 'none'; form-action 'self'; upgrade-insecure-requests/
  );
